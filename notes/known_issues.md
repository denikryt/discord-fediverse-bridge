...
- `/subscribe-channel` can discover same-instance bridge-owned local communities and persist local subscriber rows. Stage 4 makes active local subscriber forums source-capable for new posts/comments.
...
- Stage 4 local subscriber fanout uses missing surface rows as the retry signal; there is no dedicated local Discord delivery retry table yet. Local-subscriber-originated edit/delete propagation remains deferred to Stage 5.
