...
def build_report(
    results: Sequence[Any], required_rules: Sequence[Any]
) -> dict[str, Any]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(".artifacts/test-assurance/community-management/report.json"),
    )
    ...
    from support.community_management_contracts import (
        ...
        REQUIRED_COMMUNITY_MANAGEMENT_RULES,
    )

    collector = PassiveCaseCollector(
        accepts=lambda case: isinstance(case, CommunityManagementCase)
    )
    exit_code = pytest.main(
        ["-q", "tests/operations/test_community_management_contract_cases.py"],
        plugins=[collector],
    )
    ...
    output.write_text(
        json.dumps(
            build_report(collector.results(), REQUIRED_COMMUNITY_MANAGEMENT_RULES),
            indent=2,
            sort_keys=True,
        )
        + "\n"
