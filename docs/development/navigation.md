...
1. `src/commands/subscribe.py` for `/subscribe-community` option wiring, optional forum-channel placement, autocomplete source selection, no-ping initiator mentions in success output, and defensive raw interaction reads for `instance_domain`; 2. `src/community_labels.py` for compact `slug@instance` labels used by subscription command output; 3. `src/discord_forum_placement.py` for selected-channel availability, auto-created forum channels, Manage Channels error mapping, and best-effort cleanup; 4. `src/lemmyverse_communities.py` for disk-backed global Lemmyverse autocomplete cache, lazy background refresh, retry handling, monthly-active-user ranking, and Fediverse choice labels; 5. `src/community_discovery.py` for direct-instance autocomplete labels and selected community resolution; 6. `src/operations/subscribe.py`; 7. `src/db/repositories/remote_subscriptions.py`; 8. `src/db/repositories/bridge_actor_follows.py`; 9. `src/lemmy_client.py`; 10. `src/fedify_gateway_client.py`; 11. `fedify-gateway/src/federation-outbound.ts`.
...
1. `src/commands/create_community.py` for the `/create_community` modal launcher, modal submit validation, optional forum-channel selection, and snapshot timing; 2. `src/discord_forum_placement.py` for selected-channel availability, auto-created forum channels, Manage Channels error mapping, and cleanup after later failures; 3. `src/operations/create_community.py` for authorization/forbidden-audit precheck and domain creation; 4. `src/local_communities/service.py`; 5. `src/management_actions.py` for transactional creation plus audit; 6. `src/db/repositories/local_communities.py`; 7. `src/management_audit_recorder.py`; 8. `src/management_audit.py`; 9. `src/db/repositories/management_audit_events.py`; 10. `src/db/database.py`; 11. `src/local_community_permissions.py` for owner/super-admin management checks.
