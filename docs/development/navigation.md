...
1. `src/commands/subscribe.py` for `/subscribe-community` option wiring, optional forum-channel placement, autocomplete source selection, and defensive raw interaction reads for `instance_domain`; 2. `src/discord_forum_placement.py` for selected-channel availability, auto-created forum channels, Manage Channels error mapping, and best-effort cleanup; 3. `src/lemmyverse_communities.py` for disk-backed global Lemmyverse autocomplete cache, lazy background refresh, retry handling, monthly-active-user ranking, and Fediverse choice labels; 4. `src/community_discovery.py` for direct-instance autocomplete labels and selected community resolution; 5. `src/operations/subscribe.py`; 6. `src/db/repositories/remote_subscriptions.py`; 7. `src/db/repositories/bridge_actor_follows.py`; 8. `src/lemmy_client.py`; 9. `src/fedify_gateway_client.py`; 10. `fedify-gateway/src/federation-outbound.ts`.
