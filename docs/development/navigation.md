...
1. `src/commands/subscribe.py` for Discord option wiring, autocomplete source selection, and defensive raw interaction reads for `instance_domain`; 2. `src/lemmyverse_communities.py` for disk-backed global Lemmyverse autocomplete cache, lazy background refresh, retry handling, monthly-active-user ranking, and Fediverse choice labels; 3. `src/community_discovery.py` for direct-instance autocomplete labels and selected community resolution; 4. `src/operations/subscribe.py`; 5. `src/db/repositories/remote_subscriptions.py`; 6. `src/db/repositories/bridge_actor_follows.py`; 7. `src/lemmy_client.py`; 8. `src/fedify_gateway_client.py`; 9. `fedify-gateway/src/federation-outbound.ts`.
...
1. `src/commands/create_community.py`; 2. `src/operations/create_community.py`; 3. `src/local_communities/service.py`; 4. `src/management_actions.py` for transactional creation plus audit; 5. `src/db/repositories/local_communities.py`; 6. `src/management_audit_recorder.py`; 7. `src/management_audit.py`; 8. `src/db/repositories/management_audit_events.py`; 9. `src/db/database.py`; 10. `src/local_community_permissions.py` for owner/super-admin management checks.
