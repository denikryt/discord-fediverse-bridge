...
| `local_communities` | `LocalCommunity` | Local community hosting | Create community operation | Gateway actor store, local runtime, management authorization | Discord forum exposed as AP Group | Unique forum channel, slug, and actor URL; new rows store `created_by_discord_user_id`; legacy rows may be NULL and are super-admin-managed |
...
| `community_actor_bans` | `CommunityActorBan` | Local community moderation | `/ban-user` operation | Inbound ActivityPub moderation check | Store local-only remote actor bans scoped to one bridge-hosted community | Unique community/actor handle/status; active bans skip future inbound side effects |
...
| `discord_guild_snapshots` | `DiscordGuildSnapshot` | Discord directory snapshots | Command adapters, bot startup refresh | Dashboard | Cache public-readable guild names without live Discord lookups | One snapshot per Discord guild id; ids are not serialized publicly |
| `discord_channel_snapshots` | `DiscordChannelSnapshot` | Discord directory snapshots | Command adapters, bot startup refresh | Dashboard | Cache public-readable forum/channel names without live Discord lookups | One snapshot per Discord channel id; ids are not serialized publicly |
...
`activitypub_event_receipts`, `message_mappings`, `published_activity_objects`, `remote_actors`, Discord snapshot tables, and the community group tables are shared infrastructure for deduplication, object lookup, dashboard labeling, and fanout.
