...
class CommunityActorBanRepository(BaseRepository):
    ...
    def fill_actor_url_if_missing(self, *, ban_id: int, actor_url: str) -> None:
        ...
        with self.session() as session:
            ...
            ban.updated_at = utcnow()
            session.flush()
    def create_active_ban_with_audit(
        self,
        *,
        local_community_id: int,
        actor_handle: str,
        ...
        """Create/reactivate an active ban and write the matching audit row."""
        ...
        with self.session() as session:
            ...
            if inactive is not None:
                ...
                new_actor_url = inactive.actor_url
                if actor_url and not inactive.actor_url:
                    before["actor_url"] = inactive.actor_url
                    inactive.actor_url = actor_url
                    new_actor_url = actor_url
                inactive.status = "active"
                inactive.reason = reason
                inactive.created_by_discord_user_id = created_by_discord_user_id
                inactive.updated_at = utcnow()
                ...
                if "actor_url" in before:
                    after["actor_url"] = new_actor_url
                session.flush()
                # The audit insert is inside the same transaction as the state
                # transition so reactivation cannot commit without history.
                audit_repository.add_event(
                    session,
                    action=ACTION_BAN_REACTIVATED,
                    result=RESULT_SUCCESS,
                    actor_discord_user_id=created_by_discord_user_id,
                    local_community_id=local_community_id,
                    target_type=TARGET_REMOTE_ACTOR,
                    target_id=actor_handle,
                    before=before,
                    after=after,
                )
                return inactive
    ...
    def deactivate_active_ban_by_handle_with_audit(
        self,
        *,
        local_community_id: int,
        actor_handle: str,
        ...
        """Deactivate an active ban and audit the status transition atomically."""
        from ...management_audit import ACTION_BAN_REMOVED, RESULT_SUCCESS, TARGET_REMOTE_ACTOR
