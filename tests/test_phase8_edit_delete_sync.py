...
@pytest.mark.asyncio
async def test_mirror_message_edit_does_not_propagate(tmp_path: Path) -> None:
    """A message with role='mirror' must not trigger edit propagation."""
    database = _database(tmp_path)
    mirror_msg_id = 600
    ...
    # Verify the delivery row exists and is role='mirror'.
    delivery = database.get_message_delivery_by_message(mirror_msg_id)
    assert delivery is not None
    assert delivery.role == "mirror"

    # on_raw_message_edit should return immediately without calling handle_discord_message_edit.
    # We simulate the guard logic directly: if role == 'mirror', do not call runtime.
    # This matches the guard in discord_bot.on_raw_message_edit.
    gateway = AsyncMock()
    runtime_obj = _fake_runtime(gateway=gateway)
    community_runtime = _community_runtime(database)

    # The guard in on_raw_message_edit checks delivery.role before calling runtime.
    # We verify the guard is in place by checking the DB — no edit propagation occurs
    # because on_raw_message_edit returns before handle_discord_message_edit is called.
    if delivery.role == "mirror":
        # guard fires — do not call
        pass
    else:
        await community_runtime.handle_discord_message_edit(
...
@pytest.mark.asyncio
async def test_mirror_message_delete_does_not_propagate(tmp_path: Path) -> None:
    """A message with role='mirror' must not trigger delete propagation."""
    database = _database(tmp_path)
    mirror_msg_id = 600
