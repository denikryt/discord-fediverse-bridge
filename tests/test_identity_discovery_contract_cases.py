...
import pytest
from src.community_discovery import (
    BridgeDiscoveryUnavailable,
    ...
    if case.action == "normalize_handle":
        if case.expected.error_contains:
            with pytest.raises(InvalidRemoteActorHandle):
                normalize_remote_actor_handle(case.raw)
        else:
            assert normalize_remote_actor_handle(case.raw) == case.expected.value
    elif case.action == "extract_handle":
        assert (
            extract_remote_actor_handle_from_actor_url(case.raw) == case.expected.value
        )
        ...
        if case.raw.startswith("!"):
            ...
            actual = community_relay_label(
                actor_id=case.raw, name="Technology", handle=None
            )
